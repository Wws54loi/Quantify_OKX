...
def write_log_header(f, leverage, profit_target_percent, stop_loss_percent, initial_capital,
                     total_combinations, valid_results, high_winrate_count):
    """写入日志文件头部"""
    f.write("="*80 + "\n")
    f.write("止损延迟和K1涨跌幅参数优化结果 - 胜率>=60%的所有策略 (交易次数>=100)\n")
    f.write("="*80 + "\n")
    f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write(f"固定参数: 杠杆={leverage}x, 止盈={profit_target_percent}%, 止损={stop_loss_percent}%, 每次投入={initial_capital} USDT\n")
...
def write_strategy_detail(f, rank, result, leverage, profit_target_percent, stop_loss_percent, initial_capital):
    """写入单个策略的详细信息"""
    f.write(f"{'='*80}\n")
...
def optimize_parameters():
    ...
    # 计算总组合数
    total_combinations = len(list(stop_loss_delay_bars_range)) * len(min_k1_range_range)
    ...
    count = 0
    for delay_bars, k1_pct in itertools.product(stop_loss_delay_bars_range, min_k1_range_range):
        count += 1
    ...
    for i, result in enumerate(results[:10], 1):
        print(f"\n第 {i} 名:")
        print(f"  止损延迟: {result['stop_loss_delay_bars']}根K线 ({result['stop_loss_delay_bars']*15}分钟) | K1涨跌幅: {result['min_k1_range_percent']}%")
    ...
    with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as f:
        fieldnames = [
            '排名', '止损延迟(K线)', '止损延迟(分钟)', 'K1涨跌幅%', '总交易数', '胜率%', 
    ...
    if len(high_winrate_results) > 0:
        with open(log_filename, 'w', encoding='utf-8') as f:
            write_log_header(f, leverage, profit_target_percent, stop_loss_percent, initial_capital,
                           total_combinations, len(results), len(high_winrate_results))
            
            for i, result in enumerate(high_winrate_results, 1):
                write_strategy_detail(f, i, result, leverage, profit_target_percent, 
