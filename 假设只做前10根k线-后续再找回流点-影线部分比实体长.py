...
class ThreeKlineStrategy:
    ...
    def check_rule1(self, k1: KLine, k2: KLine, 
        ...
        参数:
            ...
            min_k2_body_percent: K2最小实体占比要求（%），默认0%
            max_k2_body_percent: K2最大实体占比要求（%），默认100%
            min_k2_shadow_percent: K2突破方向影线最小占比要求（%），默认0%
    ...
    def find_signals(self, klines: List[KLine], 
                    ...
                    stop_loss_delay_bars: int = 10,  # 前10根K线不止损
        ...
        while i < len(klines) - 2:
            ...
            if signal and entry_index:
                ...
                for j in range(entry_index, len(klines)):
                    ...
                    liquidation_threshold = -1.0 / leverage  # 止损阈值（合约亏损100%）
                    
                    # 1. 检查止损（合约亏损100%）- 所有K线都检查
                    ...
                    # 3. 10根K线后，只要有盈利就平仓
                        ...
                        # 计算能盈利的价格点
                        if direction == 'long':
                            exit_price = max(entry_price * 1.0001, current_kline.close)  # 至少0.01%盈利
                        else:
...
def main():
    ...
    min_k2_body_percent = 0.0  # K2最小实体占比要求（%）
    max_k2_body_percent = 100.0  # K2最大实体占比要求（%）
    min_k2_shadow_percent = 0.0  # K2突破方向影线最小占比要求（%）
    ...
    print(f"K2实体占比区间: {min_k2_body_percent}% - {max_k2_body_percent}%")
    print(f"K2影线突破最小占比: {min_k2_shadow_percent}%")
    ...
    signals = strategy.find_signals(klines, 
                                    ...
                                    min_k2_body_percent=min_k2_body_percent,
                                    max_k2_body_percent=max_k2_body_percent,
                                    min_k2_shadow_percent=min_k2_shadow_percent,
