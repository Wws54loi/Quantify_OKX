"""
三K线策略胜率统计 - 币安BTC 15分钟K线 (收盘价强制平仓版本)
作者: 
日期: 2025-11-04
...
平仓规则:
  - 止盈出场: 下一根K线的最高价触及止盈线时,按止盈价平仓
  - 止损出场: 下一根K线的最低价触及止损线时,按止损价平仓  
  - 收盘价出场: 下一根K线走完后,既未触发止盈也未触发止损,按收盘价强制平仓
...
class ThreeKlineStrategy:
    ...
    def find_signals(self, klines: List[KLine], min_k1_range: float = 0.005) -> List[Dict]:
        ...
        while i < len(klines) - 3:  # 确保至少有3根K线用于验证
            k1 = klines[i]
            k2 = klines[i + 1]
            k3 = klines[i + 2]
            ...
            if self.is_contained(k1, k2):
                ...
                if is_valid:
                    signal = {
                        ...
                        'exit_kline': klines[i + 3] if i + 3 < len(klines) else None
                    }
                    if signal['exit_kline']:
            ...
            # 检查法则1: k2相对于k1
                ...
                if is_valid:
                    signal = {
                        ...
                        'exit_kline': klines[i + 2] if i + 2 < len(klines) else None
                    }
                    if signal['exit_kline']:
                        signals.append(signal)
                    i += 1  # 继续检查下一组
    ...
    def calculate_win_rate(self, signals: List[Dict], 
                          profit_target: float = 0.008,  # 0.8%止盈
                          stop_loss: float = 0.004,  # 0.4%止损
                          leverage: int = 50,
        ...
        for idx, signal in enumerate(signals, 1):
            ...
            if direction == 'long':
                # 做多：价格上涨盈利，下跌亏损
                high_return = (exit_kline.high - entry_price) / entry_price
                low_return = (exit_kline.low - entry_price) / entry_price
                close_return = (exit_kline.close - entry_price) / entry_price
            else:  # short
                # 做空：价格下跌盈利，上涨亏损
                high_return = (entry_price - exit_kline.low) / entry_price
                low_return = (entry_price - exit_kline.high) / entry_price
                close_return = (entry_price - exit_kline.close) / entry_price
            ...
            if high_return >= profit_target:
                ...
                price_change_percent = profit_target * 100
                contract_return = profit_target * leverage * 100
                if direction == 'long':
                ...
                price_change_percent = -stop_loss * 100
                contract_return = -stop_loss * leverage * 100
                if direction == 'long':
                ...
                price_change_percent = close_return * 100
                contract_return = close_return * leverage * 100
                exit_price = exit_kline.close
            
            # 计算本次交易盈亏(USDT)
            pnl = initial_capital * actual_return * leverage
            total_capital += pnl
            ...
            if pnl > 0:
                ...
                losses += 1
                losses_list.append(actual_return)
            ...
            trade_detail = {
                ...
                'exit_time': datetime.fromtimestamp(exit_kline.timestamp/1000).strftime('%Y-%m-%d %H:%M'),
...
def export_to_csv(trade_details: List[Dict], filename: str = "trade_log_close_exit.csv"):
    """导出交易详情到CSV文件"""
...
def export_to_txt(trade_details: List[Dict], stats: Dict, filename: str = "trade_log_close_exit.txt"):
    """导出详细交易日志到TXT文件"""
    if not trade_details:
        print("没有交易数据可导出")
        return
    ...
    try:
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            for trade in trade_details:
                ...
                f.write(f"  持仓时长: {trade['holding_time']}\n")
...
def print_signal_details(signals: List[Dict], limit: int = 10):
    ...
    for i, signal in enumerate(signals[:limit]):
        print(f"\n信号 #{i+1} - 类型: {signal['type']}")
        print(f"  时间: {datetime.fromtimestamp(signal['entry_time']/1000).strftime('%Y-%m-%d %H:%M')}")
        print(f"  K1: {signal['k1']}")
        print(f"  K2: {signal['k2']}")
        if signal['type'] == 'rule2':
            print(f"  K3: {signal['k3']}")
        print(f"  入场价格: {signal['entry_price']:.2f}")
        if signal['exit_kline']:
            print(f"  下根K线: {signal['exit_kline']}")
...
def main():
    ...
    if os.path.exists(cache_file):
        ...
        try:
            ...
            print(f"✗ 读取缓存失败: {e}")
            print("请先运行 three_kline_strategy.py 生成缓存文件")
            return
    else:
        print(f"\n✗ 未找到缓存文件: {cache_file}")
        print("请先运行 three_kline_strategy.py 生成缓存文件")
    ...
    print(f"持仓时长: {stats['avg_holding_time']}")
