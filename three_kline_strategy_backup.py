...
class ThreeKlineStrategy:
    ...
    def find_signals(self, klines: List[KLine], 
                    ...
                    stop_loss: float = 0.004) -> List[Dict]:
        """
            ...
            if signal and entry_index:
                ...
                for j in range(entry_index, len(klines)):
                    ...
                    if high_return >= profit_target:
                        ...
                        signal['exit_index'] = j
                        signal['holding_bars'] = j - entry_index + 1
...
def main():
    ...
    # 计算止盈止损参数
    leverage = 50
    profit_target_percent = 60  # 止盈40%
    stop_loss_percent = 50  # 止损20%
    ...
    print(f"止盈设置: 40% (价格变动 0.80%)")
    print(f"止损设置: 20% (价格变动 0.40%)")
    print(f"{'-'*80}")
