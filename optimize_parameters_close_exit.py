"""
参数优化脚本 - 寻找最优的止盈、止损和K1涨跌幅参数（收盘价强制平仓版本）
"""
...
def optimize_parameters():
    ...
    print("="*80)
    print("参数优化系统 - 寻找最优止盈止损参数（收盘价强制平仓版本）")
    print("="*80)
    print(f"固定参数:")
    ...
    for profit_pct, stop_pct, k1_pct in itertools.product(profit_target_range, stop_loss_range, min_k1_range_range):
        ...
        if count % 100 == 0 or count == total_combinations:
            ...
            print(f"进度: {count}/{total_combinations} ({count/total_combinations*100:.1f}%) - 当前最佳盈亏: {best_profit:+.4f} USDT (缓存:{cache_size})", end='\r')
    ...
    filename = f"optimization_results_close_exit_{timestamp}.csv"
    logfilename = f"optimization_top10_{timestamp}.log"
    ...
    try:
        with open(logfilename, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("参数优化结果 - TOP 10 盈利策略 (收盘价强制平仓版本)\n")
            f.write("="*80 + "\n")
