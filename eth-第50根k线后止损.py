...
class BinanceAPI:
    ...
    @staticmethod
    def get_klines(symbol: str = "ETHUSDT", interval: str = "15m", limit: int = 1000) -> List[List]:
        """
        获取K线数据（支持获取超过1000根）
...
def main():
    ...
    if raw_klines is None:
        print("\n正在从币安API获取K线数据...")
        api = BinanceAPI()
        raw_klines = api.get_klines(symbol="ETHUSDT", interval="15m", limit=30000)
