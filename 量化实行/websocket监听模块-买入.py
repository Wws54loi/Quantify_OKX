	...
	with open(log_file, 'a', newline='', encoding='utf-8') as f:
		...
		if not file_exists:
			writer.writerow([
				...
				'是否平仓','出场时间','出场价格','持仓K线数','持仓时长','价格变动%','合约收益%','盈亏USDT',
				'备注'
			])
		writer.writerow([
			trade_id,
								...
								if time_remaining <= 5:
									...
									if still_in_range:
										...
										if body_ratio_valid:
											...
											# 风险控制：仅允许同时一笔持仓；且仅在高效时段允许买入
											open_cnt = count_open_positions()
											if open_cnt is None:
												print("⚠ 无法读取未平仓数量（文件占用或异常），为安全起见跳过本次买入")
												signal_recorded = True
												continue
											# 只允许一笔持仓：已有任意未平仓则跳过
											if open_cnt >= 1:
												print("⛔ 跳过买入：当前已有未平仓数量为", open_cnt, "，规则限制为同时仅一笔。")
												print(f"   拟交易方向: {trade_direction} | 拟入场价: {entry_price:.2f}")
												signal_recorded = True
												continue
											# 限定高效时间段
											if not is_in_efficient_time():
												print("⏱ 非高效时间段，跳过本次买入")
												print(f"   拟交易方向: {trade_direction} | 拟入场价: {entry_price:.2f}")
												signal_recorded = True
												continue
