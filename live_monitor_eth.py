...
"""
    ...
    def send_notification(self, signal: Dict, show_popup: bool = False):
        ...
        try:
            ...
            title = f"⚠️ {direction}信号 - ETH 15分钟策略"
        ...
        try:
            for i in range(10):
                if i % 2 == 0:
                    ...
                    ctypes.windll.kernel32.SetConsoleTitleW(f"实时监听器 - ETH 15分钟策略")
                time.sleep(0.3)
            # 恢复原标题
            ctypes.windll.kernel32.SetConsoleTitleW("实时监听器 - ETH 15分钟策略")
        except:
            pass
    
    def update_15m_kline(self):
        """更新15分钟K线数据"""
        klines_15m = self.api.get_latest_klines(symbol="ETHUSDT", interval="15m", limit=2)
        if len(klines_15m) < 2:
            if len(klines_15m) == 0:
    ...
    def run(self, check_interval: int = 10):
        ...
        """
        print("="*80)
        print(f"实时监听器启动 | 交易对:ETHUSDT | K1涨跌幅>={self.min_k1_range*100:.2f}% | 间隔:{check_interval}秒")
        print("="*80)
